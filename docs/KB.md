...
- `python deployment/launch_batched_inference.py --submit-controller --orchestration-root output/batches/folder1_batches`: submit only the existing controller Slurm script for a previously created batch orchestration root
- `python deployment/merge_shard_unions.py --output-dir output/batches/folder1_batches/merged output/batches/folder1_batches/runs/batch_000 output/batches/folder1_batches/runs/batch_001`: merge the 4 union families from completed batch runs into one final mosaic output; inputs may cover different extents but must share CRS and pixel size
