...
- `deployment/`: cluster-facing orchestration helpers such as tile-list generation, union-raster merge, and the Slurm batch-per-job launcher.
...
- it now also accepts `--config <path>` so generated batch workers can run against explicit config copies instead of the repo root `config.yml`
...
- `workflows/inference_only.py`:
  ...
  - can now take an exact one-tile-per-line generated tile file via `io.inference.tiles_file`, which bypasses folder globbing and source-label refiltering inside the worker run
...
3. The bootstrap stage configures logging, loads processed holdout tiles, restores time-budget state, resolves the shared holdout tile grid, and opens rolling union rasters.
...
6. Holdout inference updates union rasters and processed-tile logs tile by tile, so partial runs keep usable outputs.
7. Disk feature caches are consolidated after successful workflows that need them.
8. Large folder inference can be parallelized by building fixed-size batch tile files, launching one ordinary Slurm job per batch, retrying incomplete runs against the same fixed run directories, and merging the resulting union families after verification succeeds.
