...
- Tile mode switch:
  ...
  - `io.auto_split.enabled=false`: uses explicit manual lists from `io.paths.source_tiles` (or `io.paths.source_tile`) and `io.paths.val_tiles`; inference tiles come from `io.paths.inference_dir` (`io.paths.inference_glob`) or fallback `io.paths.holdout_tiles`.
